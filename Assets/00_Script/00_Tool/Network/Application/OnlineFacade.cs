...
namespace GL.Network.Application
{
    public sealed class OnlineFacade
    {
        ...
        public OnlineFacade(ILobbyService lobby, ITransport transport)
        {
            _lobby = lobby;
        ...
        public void Tick()
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                {
                    var text = Encoding.UTF8.GetString(r.Envelope.Payload.Span);
                    var name = _lobby.GetMemberDisplayName(r.From);
                    AddLine($"{name}: {text}");
                }
            }
        }
        ...
        {
            ChatLog.Add(s);
            if (ChatLog.Count > 200) ChatLog.RemoveRange(0, ChatLog.Count - 200);
        }
    }
}
